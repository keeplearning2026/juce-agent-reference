...
def parse_index(index_path: Path) -> list[CompoundIndexEntry]:
    """Parse the Doxygen ``index.xml`` and return all compound entries.

    Args:
        index_path: Path to ``index.xml``.

    Returns:
        List of ``CompoundIndexEntry`` in file order.

    Raises:
        ConversionError: If the XML is malformed or missing required fields.
    """
    if not index_path.is_file():
        raise ConversionError(
            f"index.xml not found: {index_path}",
    ...
    for elem in root.iterfind("compound"):
        ...
        if not refid:
            raise ConversionError(
                "Compound in index.xml missing refid",
                file_path=str(index_path),
                xml_tag="compound",
            )
...
# Member kinds that produce individual members (not skipped silently).
...
def parse_compound(xml_path: Path) -> Compound:
    """Parse a single compound XML file into a ``Compound``.

    Args:
        xml_path: Path to ``<refid>.xml``.

    Returns:
        A fully populated ``Compound``.

    Raises:
        ConversionError: If the XML is malformed.
        UnsupportedSemanticNodeError: If a semantic node cannot be handled.
    """
    if not xml_path.is_file():
        raise ConversionError(
            f"Compound XML not found: {xml_path}",
    ...
    try:
        ...
        raise ConversionError(
            f"Malformed compound XML {xml_path.name}: {exc}",
            file_path=str(xml_path),
        ) from exc
    ...
    if cdef is None:
        raise ConversionError(
            f"No <compounddef> in {xml_path.name}",
            file_path=str(xml_path),
        )
    ...
    qualified_name = name  # Doxygen already qualifies it in compoundname.
    ...
    for belem in cdef.iterfind("basecompoundref"):
        bases.append(Reference(
            text=(belem.text or "").strip(),
            refid=belem.get("refid"),
            kind=None,
        ))
    ...
    for delem in cdef.iterfind("derivedcompoundref"):
        derived.append(Reference(
            text=(delem.text or "").strip(),
            refid=delem.get("refid"),
            kind=None,
        ))

    # Inner compounds (nested classes, inner namespaces)
    inner_compounds: list[Reference] = []
    for icelem in cdef.iterfind("innerclass"):
        inner_compounds.append(Reference(
            text=(icelem.text or "").strip(),
            refid=icelem.get("refid"),
            kind="class",
        ))
    ...
    if loc_elem is not None:
        location = SourceLocation(
            file=loc_elem.get("file", ""),
            line=_parse_int(loc_elem.get("line")),
    ...
    # Module (from Doxygen group)
    module: str | None = None
    for grp in cdef.iterfind("ingroup"):
        mid = grp.get("refid", "")
        if mid:
            module = mid  # Will be resolved later by the path mapper.
    ...
    for sdef in cdef.iterfind("sectiondef"):
        ...
        if member_refs:
            sections.append(CompoundSection(
                kind=skind,
                title=stitle,
                member_refids=tuple(member_refs),
            ))

    # Disposition
    documented = (
        cdef.get("prot", "public") != "private"
        or brief
        or details
    )
    ...
    return Compound(
        ...
        inner_compounds=tuple(inner_compounds),
...
def _parse_member(mdef: etree._Element) -> Member:
    """Parse one ``<memberdef>`` element into a ``Member``."""
    refid = mdef.get("id", "")
    kind = mdef.get("kind", "")
    ...
    # Definition (return type + name + params)
    ...
    for pelem in mdef.iterfind("param"):
        ...
        parameters.append(Parameter(
            ...
            name=pname,
            default_value_nodes=defval_nodes,
    ...
    # Enum values (embedded in enum members)
    if kind == "enum":
        for ev_elem in mdef.iterfind("enumvalue"):
            ...
            # We add enum values as special parameters
            parameters.append(Parameter(
                name=ev_name,
                description=tuple(ev_brief),
            ))
    ...
    return Member(
        ...
        args_string_nodes=args_string_nodes,
        signature=signature,
        access=prot,
        static=static,
        const=const,
...
def _parse_doc_children(
    parent: etree._Element | None,
    *,
    default: tuple[DocNode, ...] = (),
) -> tuple[DocNode, ...]:
    """Parse all children of *parent* into DocNode tuples.

    Each top-level element becomes a paragraph (when it contains text).
    Nested elements are recursively parsed.
    """
    if parent is None:
        return default
...
def _parse_doc_paragraphs(parent: etree._Element | None) -> list[DocNode]:
    """Parse a briefdescription / detaileddescription, producing a flat list
    of paragraphs and other block elements.
    """
    result = _parse_doc_children(parent)
    return list(result)


def _parse_doc_element(elem: etree._Element) -> DocNode | list[DocNode] | None:
    """Dispatch a single documentation XML element to the right DocNode."""
    tag = elem.tag
    ...
    if tag == "para":
        ...
        # Capture text before first child.
    ...
    if tag == "xrefsect":
        # Cross-reference section (e.g. @deprecated)
    ...
    if tag == "sect1":
        ...
        return Section(level=1, title=sect_title, children=body)

    if tag == "sect2":
        sect2_title: tuple[DocNode, ...] = (
            tuple(_parse_inline_children(elem.find("title")))
            if elem.find("title") is not None else ()
        )
        body2 = _parse_doc_children(elem, default=())
        return Section(level=2, title=sect2_title, children=body2)

    if tag == "sect3":
        sect3_title: tuple[DocNode, ...] = (
            tuple(_parse_inline_children(elem.find("title")))
            if elem.find("title") is not None else ()
        )
        body3 = _parse_doc_children(elem, default=())
        return Section(level=3, title=sect3_title, children=body3)

    if tag == "sect4":
        sect4_title: tuple[DocNode, ...] = (
            tuple(_parse_inline_children(elem.find("title")))
            if elem.find("title") is not None else ()
        )
        body4 = _parse_doc_children(elem, default=())
        return Section(level=4, title=sect4_title, children=body4)
    ...
    if tag == "codeline":
        return None  # handled by parent programlisting
    ...
    # Inline tags (also valid at block level in Doxygen)
    if tag == "ref":
        refid = elem.get("refid")
...
def _parse_inline_fallback(elem: etree._Element) -> list[DocNode]:
    """Graceful fallback: capture all text content into a paragraph.

    Used for nodes we don't have explicit handling for yet, preserving
    textual content rather than silently dropping it.
    """
    return [Paragraph(children=(Text("".join(itertext(elem))),))]


def itertext(elem: etree._Element) -> list[str]:
    """Yield all text content from an element tree, depth-first."""
    parts: list[str] = []
    if elem.text:
